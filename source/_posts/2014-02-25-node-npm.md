...
npm 有个 init 命令，可以引导你完成创建 package.json 文件的过程。即便你非常熟悉 package.json 和它的属性，也可以把npm init 当作将你的新程序或模块导入正轨的简便办法。 它可以聪明地为你设置默认值，比如通过上层目录的名称推断模块名，从 `~/.npmrc` 中读取创作者的信息，以及用你的 git 设置确定代码库。
